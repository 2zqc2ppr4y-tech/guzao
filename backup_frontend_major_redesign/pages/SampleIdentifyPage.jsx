  ...
  return (
    <div className="grid gap-6">
      ...
      <div className="page-header">
        <div>
          <p className="eyebrow"><ScanSearch className="h-4 w-4" />样品识别</p>
          <h1 className="mt-2 text-2xl font-bold text-white">
            {mode === 'single' ? '单张显微图像分析' : '批量图像检测'}
          </h1>
          <p className="mt-1 text-sm text-slate-400">
            {mode === 'single'
              ? '上传显微图片，YOLO11 实时检测鼓藻属类，返回置信度排序与 AI 形态解析。'
        ...
        {result ? (
          <>
            ...
            {result.detections.length > 0 && (
              <div className="mt-5">
                <h3 className="mb-3 font-bold text-white">检测框列表</h3>
                <div className="grid gap-3">
                  {result.detections.map((item, i) => (
                    <div key={`${item.label}-${i}`} className="candidate-row">
                      ...
                      <p className="mt-2 text-xs text-slate-400">bbox: [{item.bbox.map((v) => Number(v).toFixed(1)).join(', ')}]</p>
                      <ProgressBar value={item.confidence} color={item.confidence < 0.8 ? '#D97706' : '#1CA78A'} />
                    </div>
                  ))}
                </div>
          ...
          <div className="no-detection mt-5">上传图片并点击"开始识别"后，这里会显示带框结果图和检测列表。</div>
        )}
      </Panel>
    ...
    <div className="grid gap-6">
      ...
      <section className="grid gap-6 lg:grid-cols-[320px_1fr]">
        ...
        <div className="grid gap-6 content-start">
            ...
            <>
              <section className="grid gap-4 sm:grid-cols-3">
                <Panel>
                  ...
                  <span className="mt-1 block text-xs text-slate-400">批次整体模型稳定性</span>
                </Panel>
                <Panel>
                  <p className="text-sm text-slate-300">低置信度比例</p>
                  <strong className="mt-2 block text-3xl text-amber-200">{formatPercent(summary.low / Math.max(1, summary.total))}</strong>
                  <span className="mt-1 block text-xs text-slate-400">自动进入待复核</span>
                </Panel>
                <Panel>
                  <p className="text-sm text-slate-300">任务编号</p>
                  <strong className="mt-2 block text-lg text-white break-all">{taskId || '待运行'}</strong>
                  <span className="mt-1 block text-xs text-slate-400">可用于报告归档</span>
                </Panel>
              </section>
          ...
          {!items.length && (
            <Panel>
              <p className="text-slate-300 text-sm">选择图片后点击"开始批量检测"，结果将在此显示。</p>
            </Panel>
          )}
        </div>
      </section>

import { CheckCircle2, CloudOff, Eye, EyeOff, Loader2, LogIn, LogOut, UserPlus, UserRound, X } from 'lucide-react'
import { useEffect, useMemo, useState } from 'react'
import { loginUser, logoutUser, registerUser } from '../api'
import { setAuth } from '../auth'

export default function Layout({ navItems, active, backend, auth, onAuthChange, onNotify, onNavigate, brandIcon: BrandIcon, children }) {
  const [authOpen, setAuthOpen] = useState(false)

  const backendInfo = useMemo(() => {
    if (backend?.status === 'up') return { label: '服务在线', className: 'is-online', icon: CheckCircle2 }
    if (backend?.status === 'down') return { label: '离线模式', className: 'is-offline', icon: CloudOff }
    return { label: '检查服务', className: 'is-checking', icon: Loader2 }
  }, [backend?.status])
  ...
  return (
    <div className="app-shell">
      <header className="top-nav">
        <div className="top-nav-inner mx-auto flex max-w-[1360px] items-center justify-between gap-3 px-4">
          <button className="brand-button" onClick={() => onNavigate('home')}>
            ...
            <span>
              <strong>鼓藻鉴析</strong>
              <small>YOLO11 水生态智能识别</small>
            </span>
          </button>

          <nav className="nav-scroll hidden flex-1 items-center justify-center gap-1 lg:flex" aria-label="主导航">
            {navItems.map((item) => {
          ...
          <div className="account-area">
            <span className={`backend-chip ${backendInfo.className}`} title={backend?.error || backendInfo.label}>
              <BackendIcon className="h-4 w-4" />
              <span>{backendInfo.label}</span>
            </span>
            <button className={`account-button ${active === 'profile' ? 'account-button-active' : ''}`} onClick={openProfile}>
        ...
        <div className="mobile-nav mx-auto flex max-w-[1360px] gap-2 overflow-x-auto px-4 pb-3 lg:hidden" aria-label="移动端导航">
          {navItems.map((item) => {
            const Icon = item.icon
            return (
              <button

...
import { DataTable, EmptyState, formatPercent, Panel, StatCard, StatusPill } from '../components/ui'
  ...
  return `
    <div style="font-family:'Microsoft YaHei',Arial,sans-serif;color:#1F2937;max-width:680px;margin:0 auto;padding:2rem;background:#fff;">
      ...
      <div style="text-align:center;padding-bottom:1.25rem;border-bottom:2.5px solid #1CA78A;margin-bottom:1.5rem;">
        <div style="font-size:.75rem;color:#0F766E;font-weight:700;letter-spacing:.08em;text-transform:uppercase;margin-bottom:.3rem;">鼓藻鉴析 · YOLO11 水生态智能识别</div>
      ...
      <div style="margin-top:1.5rem;padding:.75rem;border-top:1px solid #E5E7EB;font-size:.76rem;color:#9CA3AF;text-align:center;line-height:1.7;">
        本报告由鼓藻鉴析基于 YOLO11 图像识别模型自动生成，仅用于科研辅助识别与样品初筛，<br/>
        ...
        <div className="report-modal-body">
          ...
          <div className="report-preview">
            ...
            <div className="report-footer-note">
              本报告由鼓藻鉴析基于 YOLO11 图像识别模型自动生成，仅用于科研辅助识别与样品初筛，
              最终鉴定结论请结合人工鉴定和实验室复核，不应单独作为科学论文或监管决策的依据。
            </div>
          </div>
        </div>
...
export default function ProfilePage({ auth, onAuthChange, onNotify }) {
    ...
    return (
      <div className="grid gap-6">
        <div className="page-header">
          <div>
            <p className="eyebrow"><UserRound className="h-4 w-4" />检测记录</p>
            <h1>请先登录账户</h1>
  ...
  const FILTER_TABS = [
    { label: '全部', count: allRecords.length },
    { label: '已识别', count: allRecords.filter((r) => Number(r.confidence || 0) >= 0.8).length },
    { label: '待复核', count: allRecords.filter((r) => { const c = Number(r.confidence || 0); return c > 0 && c < 0.8 }).length },
    { label: '低置信度', count: allRecords.filter((r) => Number(r.confidence || 0) > 0 && Number(r.confidence || 0) < 0.7).length },
  ]

  const filteredRecords = useMemo(() => {
    let list = allRecords
    if (filterTab === '已识别') list = list.filter((r) => Number(r.confidence || 0) >= 0.8)
    else if (filterTab === '待复核') list = list.filter((r) => { const c = Number(r.confidence || 0); return c > 0 && c < 0.8 })
    else if (filterTab === '低置信度') list = list.filter((r) => Number(r.confidence || 0) > 0 && Number(r.confidence || 0) < 0.7)
    if (searchQuery.trim()) {
      const q = searchQuery.trim().toLowerCase()
      list = list.filter((r) => [r.species, r.sample_id, r.inference_mode].join(' ').toLowerCase().includes(q))
    }
    return list
  ...
  return (
    <div className="grid gap-6">
      ...
      <div className="page-header">
        <div>
          <p className="eyebrow"><UserRound className="h-4 w-4" />检测记录</p>
          <h1>{displayName} 的记录与报告中心</h1>
          <p>集中管理当前账户的鼓藻识别记录、置信度统计、复核状态与报告导出。</p>
        </div>
      ...
      <div className="panel" style={{ padding: 0, overflow: 'hidden' }}>
        ...
        <div className="profile-filter-row">
          <div className="filter-chip-group">
            {FILTER_TABS.map((tab) => (
              <button
                key={tab.label}
                className={`filter-chip${filterTab === tab.label ? ' filter-chip-active' : ''}`}
          ...
          <div className="profile-search-wrap">
            <Search className="h-3.5 w-3.5 flex-shrink-0" style={{ color: 'var(--text-3)' }} />
            <input
              value={searchQuery}
        ...
        <div className="px-4 pb-4 pt-3">
          {loading ? (
            ...
            <EmptyState
              title={allRecords.length === 0 ? '暂无检测记录' : '没有匹配记录'}
              text={allRecords.length === 0 ? '上传一张显微图像并完成识别后，记录会自动显示在这里。' : '请调整筛选状态或搜索关键词后再查看。'}
            />
          )}
        </div>
      </div>
      ...
      <Panel>
        ...
        {recentReports.length ? (
          ...
          <EmptyState
            title="暂无已生成报告"
            text="在识别记录后点击生成报告即可创建，报告记录会保存在本地浏览器中。"
          />
        )}
      </Panel>

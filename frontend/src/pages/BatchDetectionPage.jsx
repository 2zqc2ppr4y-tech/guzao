        ...
        <div className="grid gap-6">
          <section className="grid gap-4 lg:grid-cols-3">
            <Panel>
              ...
              <span className="mt-2 block text-sm" style={{ color: 'var(--text-3)' }}>批次整体模型稳定性参考</span>
            </Panel>
            <Panel>
              <p className="text-sm" style={{ color: 'var(--text-2)' }}>低置信度样本比例</p>
              <strong className="mt-2 block text-3xl" style={{ color: 'var(--warning)' }}>{formatPercent(summary.low / Math.max(1, summary.total))}</strong>
              <span className="mt-2 block text-sm" style={{ color: 'var(--text-3)' }}>自动进入待复核</span>
            </Panel>
            <Panel>
              <p className="text-sm" style={{ color: 'var(--text-2)' }}>任务编号</p>
              <strong className="mt-2 block text-xl" style={{ color: 'var(--text)' }}>{taskId || batchTask.id}</strong>
              <span className="mt-2 block text-sm" style={{ color: 'var(--text-3)' }}>可用于报告归档</span>
            </Panel>
          </section>

...
EMU = 914400
BG_NAME = "Codex Editorial Noncard Imagegen Background"
WASH_NAME = "Codex Editorial Noncard Readability Wash"
DETAIL_NAME = "Codex Editorial Noncard Detail"
...
PATH_SLIDES = {2, 5, 7, 8, 10, 14, 16, 18}
...
def add_section_spine(sp_tree: ET.Element, sid: int, slide_idx: int, font: str) -> int:
    if slide_idx not in PATH_SLIDES:
        return sid
    y = 9.92 if slide_idx in {2, 5, 7, 8, 16, 18} else 1.34
    x0 = 1.02
    x1 = 18.65
    add_line(sp_tree, sid, x0, y, x1 - x0, 0.024, PALETTE["teal2"])
    sid += 1
    for i in range(6):
        x = x0 + i * ((x1 - x0) / 5)
        add_shape(sp_tree, sid, "ellipse", x - 0.055, y - 0.055, 0.11, 0.11, PALETTE["teal2"], 80000, PALETTE["teal2"], 78000, width=4000)
        sid += 1
    add_text(sp_tree, sid, [f"FLOW {slide_idx:02d}"], x1 - 1.10, y + 0.12, 0.90, 0.12, font, PALETTE["teal"], 5, True, "r")
    sid += 1
    return sid


def add_margin_annotation(sp_tree: ET.Element, sid: int, slide_idx: int, font: str) -> int:
    if slide_idx not in HUMAN_NOTES:
        return sid
    title, lines = HUMAN_NOTES[slide_idx]
    if slide_idx in {1, 2, 5, 10, 18}:
        x, y, w = (0.86, 8.24, 4.8) if slide_idx != 10 else (13.72, 8.30, 4.75)
    else:
        x, y, w = NOTE_POS.get(slide_idx, (14.28, 8.52, 4.60))
    add_line(sp_tree, sid, x, y + 0.02, 0.045, 0.62, PALETTE["amber"])
    sid += 1
    add_text(sp_tree, sid, [title], x + 0.16, y, w - 0.2, 0.22, font, PALETTE["deep"], 9, True)
    sid += 1
    add_text(sp_tree, sid, [f"{lines[0]} / {lines[1]}"], x + 0.16, y + 0.34, w - 0.2, 0.18, font, PALETTE["deep2"], 8, False)
    sid += 1
    add_line(sp_tree, sid, x + 0.16, y + 0.70, min(2.1, w - 0.35), 0.026, PALETTE["teal2"])
    sid += 1
    return sid
...
def add_human_details(sp_tree: ET.Element, slide_idx: int, font: str) -> None:
    ...
    sid = add_section_spine(sp_tree, sid, slide_idx, font)
    sid = add_margin_annotation(sp_tree, sid, slide_idx, font)
...
def soften_existing_cards(sp_tree: ET.Element) -> None:
    for shape in sp_tree.findall("p:sp", NS):
        ...
        srgb = sp_pr.find("a:solidFill/a:srgbClr", NS)
        if srgb is None:
            continue
        rgb = _rgb_tuple(srgb.get("val", ""))
        if rgb is None:
            continue
        if min(rgb) < 218:
            continue

        srgb.set("val", PALETTE["cream"])
        for alpha in list(srgb.findall("a:alpha", NS)):
            srgb.remove(alpha)
        ET.SubElement(srgb, qn("a", "alpha"), {"val": "22000" if area > 3 else "38000"})

        for effect_tag in ("effectLst", "effectDag"):
            _remove_child(sp_pr, sp_pr.find(f"a:{effect_tag}", NS))

        old_ln = sp_pr.find("a:ln", NS)
        _remove_child(sp_pr, old_ln)
        add_line_style(sp_pr, PALETTE["line"], 0 if area > 3 else 18000, 3600)
...
def overlay_for_key(key: str) -> tuple[str, int]:
    if key == "cover":
        return PALETTE["deep"], 7000
    if key == "data":
        return PALETTE["white"], 26000
    if key == "section":
        return PALETTE["white"], 24000
    return PALETTE["white"], 25500
...
def main() -> None:
    ...
    imagegen_bg = assets_dir / "imagegen-editorial-noncard-bg.png"
    bg_files = {
        "cover": imagegen_bg,
        "section": imagegen_bg,
        "content": imagegen_bg,
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        media_targets = {
            "cover": "ppt/media/codex_v6_imagegen_bg_cover.png",
            "section": "ppt/media/codex_v6_imagegen_bg_section.png",
            "content": "ppt/media/codex_v6_imagegen_bg_content.png",
            "data": "ppt/media/codex_v6_imagegen_bg_data.png",
        }
